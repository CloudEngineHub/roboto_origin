...
class ArmController(Node):
    ...
    def send_commands(self, sol_q, sol_tauff):
        ...
        # 10个关节名称：前5左臂，后5右臂
        msg.name = [
            "left_motor0", "left_motor1", "left_motor2", "left_motor3", "left_motor4",
            "right_motor0", "right_motor1", "right_motor2", "right_motor3", "right_motor4"
        ]
        ...
        # 计算最终位置
        msg.position = (sol_q - total_offset).tolist()
    ...
    def get_current_dual_arm_q(self):
        with self.lock:
            ...
            if self.left_arm_states is None or self.right_arm_states is None:
                logger_mp.debug("Joint states not received yet.")
                return np.zeros(10)
            ...
            try:
                ...
                logger_mp.error(f"Error parsing joint states: {e}")
                return np.zeros(10)
...
if __name__ == '__main__':
    ...
    try:
        ...
        while running:
            ...
            logger_mp.debug(f"ik:\t{round(time_ik_end - time_ik_start, 6)}s")
