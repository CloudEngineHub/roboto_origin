...
class TeleVuer:
    def __init__(self, use_hand_tracking: bool, cert_file=None, key_file=None, ngrok=False, webrtc=False):
        """
        ...
        if cert_file is None:
            cert_candidate = repo_root / "cert.pem"
            cert_file = str(cert_candidate if cert_candidate.exists() else package_dir / "cert.pem")
        if key_file is None:
            key_candidate = repo_root / "key.pem"
            key_file = str(key_candidate if key_candidate.exists() else package_dir / "key.pem")
