...
- **不要**向 `roboto_origin` 提交模块级 Pull Request 或 Issue
- **请**将模块级贡献提交到对应的子仓库
- **请**仅将快照同步问题、根目录文档、链接、仓库元数据或同步流程修复等聚合层维护事项提交到 `roboto_origin`
- 主仓库会自动从子仓库更新快照
...
仅在以下聚合层问题中使用 `roboto_origin` Issues：
...
直接提交到 `roboto_origin` 聚合层的新贡献，除非目标文件另有说明，按 GNU GPL 第 3 版接收。模块级贡献必须提交到对应的来源仓库，并遵循该仓库适用的贡献规则和许可证条款。本规则仅面向未来，不会对现有模块快照或第三方内容进行重新许可。详见 [LICENSES.md](LICENSES.md)。
...
**请记住：** 模块级贡献必须提交到相应的子仓库。主快照仓库只接收聚合层维护变更。
